            ...
            <div className={`space-y-5`}>
                <Card className={`p-5 space-y-5`}>
                    <div className={`flex items-center justify-between space-x-5`}>
                        <h1 className={`text-2xl font-medium max-w-4/5`}>{product_category?.name}</h1>
                        <EditGeneral
                        ...
                        >
                            <div className={`w-32 cursor-pointer border p-2 rounded text-sm`}><span>Chỉnh sửa</span>
                            </div>
                        </EditGeneral>
                    </div>
                    <div className={`text-end space-y-3`}>
                        ...
                        <div className={`flex items-start justify-between`}>
                            ...
                            <p className={`text-sm`}>{product_category?.is_active ? "Đang hoạt động" : "Không hoạt động"}</p>
                        </div>
                        <div className={`flex items-start justify-between`}>
                            <Label>Ngày tạo</Label>
                            <p className={`text-sm`}>{formatDate(product_category?.created_at)}</p>
                        </div>
                        <div className={`text-start`}>
                            <Label>Mô tả sản phẩm</Label>
                            <Spoiler maxHeight={100} showLabel="Show more" hideLabel="Hide">
                                <div className={`text-sm`}
                                     dangerouslySetInnerHTML={{__html: product_category?.description}}/>
                            </Spoiler>
                        </div>
                    </div>
                </Card>
                <Card className={`p-5 space-y-5`}>
                    ...
                    <div className={`space-y-5`}>
                        {
                            product_category?.category_children?.length > 0 ? product_category?.category_children.map((category) => (
                                <div key={category.id} className={`flex items-center justify-between`}>
                                    ...
                                    >
                                        <div className={`cursor-pointer border p-2 rounded text-sm`}><Pencil size={15} />
                                        </div>
                                    </EditGeneral>
                                </div>
